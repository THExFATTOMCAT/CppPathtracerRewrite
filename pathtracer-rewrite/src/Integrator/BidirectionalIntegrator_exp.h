...
public:
    ...
    void Ready(){
        ...
        std::cout << "READY\n";
        render_buffer = (scalar *) std::malloc(sizeof (scalar)*cam->res_x*cam->res_y*3);
        std::cout << "READY\n";
        int i = 0;
...
void BidirectionalIntegrator_exp::UpdateResult(){
    int i = 0;
    Camera * cam = &scene->cameras[scene->active_camera];
    //mutex->lock();
    while(i < cam->res_x*cam->res_y*3){
        cam->renderbuffer[i] += render_buffer[i];
        i ++;
    }
    //mutex->unlock();
}
...
void BidirectionalIntegrator_exp::Integrate(int x, int y) { //some variable seems to be incorrectly initialized, might have to do with vertex.end
    ...
    while(s < samples){
        ...
        cam->SampleRay(x, y, &c_ray, NULL);

        vCpy(c_vertices[0].pos, c_ray.o);
        c_vertices[0].camera = true;
        c_vertices[0].end = false;
        c_length = BuildVertices(c_vertices+1, camera_depth, &c_ray, scene, 1, mask) + 1;
        ...
        l_length = BuildVertices(l_vertices+1, light_depth, &l_ray, scene, 1, mask) + 1;
            ...
            while(c < c_length) {
                if(c_vertices[c].camera){
                    ...
                    if(cam->ProjectOnPlane(l_vertices[l].pos, uvs, &shadow_ray, &length)){
                        ...
                        if(hit.hit == false){
                            //record at uvs
                            U = (int)fmin(cam->res_x-1, cam->res_x*uvs[1]);
                            V = (int)fmin(cam->res_y-1, cam->res_y*uvs[0]);
                            vCpy(buffer, l_color);
                            vMulF(buffer, l_vertices[l].GetPdf(shadow_ray.d));
                            vDivF(buffer, length*length);
                            vDivF(buffer, (scalar) samples*l_length );
                            vAdd(&render_buffer[3*(cam->res_x*V + U)], buffer);
                        }
                    ...
                    vAdd(&render_buffer[3*(cam->res_x*y + x)], result);
                }
                if(c_vertices[c].end){
                    break;
                }
